...
if(file_exists($user_file_path)){	
	...
	foreach ($sheetDataResult as $sheetname=>$sheetdata) {			
		...
		foreach ($sheetdata as $data) {
			...
			<input type="text" name="param_val[user]['.$sheetname.'][B'.$sheet_row.']" value="'.$data['B'].'" id="'.$sheetname.'" size="29" />
			</td>';
...
if(file_exists($vmg_file_path)){	
	...
	foreach ($sheetDataResult as $sheetname=>$sheetdata) {	
		...
		foreach ($sheetdata as $data) {
			...
			<input type="text" name="param_val[vmg]['.$sheetname.'][B'.$sheet_row.']" value="'.$data['B'].'" id="'.$sheetname.'" size="29" />
			</td>';
...
function loadcfg(vmg){		
        ...
        if(retVal1){
        	window.location.href='configuration-v2.php?cfg='+vmg.value;
		}
        else{
        	window.location.href='configuration-v2.php?cfg=""';
        }
}
/*function saveCfg(){
    ...
    <table width="700" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td align="left" valign="top">
                <form id ="configwrite" name="f1" action="writeconfig.php" method="post" enctype="multipart/form-data" >
                ...
                <tr><td colspan="4" align="center"><input type="button" name="submit_btn" value="Update"  /></td></tr>                                    
        </table>
                </form>
        </td>
      </tr>
